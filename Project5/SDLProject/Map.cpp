...
Map::Map(int width, int height, unsigned int *levelData, GLuint textureID, float tile_size, float tile_count_x, float tile_count_y)
{
    this->width = width;
    this->height = height;
...
void Map::Build()
{
    for(int y = 0; y < this->height; y++) {
        for(int x = 0; x < this->width; x++) {
            ...
            if (tile == 0 || tile == 4 || tile == 5 || tile == 7) continue;
...
bool Map::IsSolid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    if (tile_x < 0 || tile_y >= height) return false;
    
    int tile = levelData[tile_y * width + tile_x];
    if (tile == 0 || tile == 4 || tile == 5 || tile == 7) return false;
